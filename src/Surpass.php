...
class Surpass {
    ...
    private $_table_name = 'image_files';
    ...
    public function path($_table_name, $path) {
    ...
    public function saveAttributes($id, $attributes) {
        ...
        return DB::table($this->_table_name)->where('id', $id)->update([
    ...
    public function removeById($ids = '') {
        ...
        try {
            ...
            foreach ($ids as $id) {
                ...
                $db = DB::table($this->_table_name)->where('id', $id);
    ...
    public function refresh() {
        ...
        try {
            ...
            $image_files = DB::table($this->_table_name)->select('id', 'dir', 'filename')->get();
            $exists_image_paths = [];
    ...
    public function load($ids=[], $old_flag=true) {
        ...
        if(!empty($ids)) {
            ...
            $this->_load = [];
            $image_files = DB::table($this->_table_name)
    ...
    private function saveData($filename, $extension, $size, $attributes) {
        ...
        if($this->isOverwrite()) {
            ...
            DB::table($this->_table_name)->where('id', $id)->update($save_params);

        } else {

            $id = DB::table($this->_table_name)->insertGetId($save_params);
